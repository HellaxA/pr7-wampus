...
public class Wumpus extends JPanel {
    ...
    private List<Integer> wasNearWumpus;
    private int prevRoom = -1;

    private List<Integer> wumpusMightBe;
    ...
    public void moveTo() {
        ...
        List<Integer> availableRooms = getListWithoutPrev();
        System.out.println("Available rooms: " + availableRooms);

        //get all tips
        Set<Hazard> set = new HashSet<>();
        for (int i = 0; i < availableRooms.size(); i++) {
            set.addAll(hazards[availableRooms.get(i)]);
        }
        System.out.println(set);
        if (set.contains(Hazard.Wumpus) && !wasNearWumpus.contains(currRoom)) {
            wumpusMightBe.addAll(availableRooms);
            wasNearWumpus.add(currRoom);

            Optional<Integer> value = checkIfHasEqual(wumpusMightBe);

            if (value.isPresent()) {
                shoot(value.get());
            } else {
                int tempPrevRoom = prevRoom;

                prevRoom = currRoom;
                currRoom = tempPrevRoom;
            }

        } else if (set.contains(Hazard.Pit)) { // go back
            int tempPrevRoom = prevRoom;
            prevRoom = currRoom;
            currRoom = tempPrevRoom;

        } else if (set.isEmpty() || set.contains(Hazard.Bat)) { // go to some pos
    ...
    private Optional<Integer> checkIfHasEqual(List<Integer> wumpusMightBe) {

        List<Integer> list = new ArrayList<>(wumpusMightBe.size());
        Set<Integer> set = new HashSet<>(wumpusMightBe.size());

        for (int i = 0; i < wumpusMightBe.size(); i++) {
            list.add(wumpusMightBe.get(i));
            set.add(wumpusMightBe.get(i));
    ...
    private List<Integer> getListWithoutPrev() {
        int availableRoomsLength = links[currRoom].length;

        List<Integer> availableRooms = new ArrayList<>(availableRoomsLength - 1);

        for (int i = 0; i < availableRoomsLength; i++) {
            if (prevRoom != links[currRoom][i]) {
                availableRooms.add(links[currRoom][i]);
            }
        }
    ...
    void startNewGame() {
        ...
        wasNearWumpus = new ArrayList<>();
        prevRoom = -1;
        numArrows = 300;
        currRoom = rand.nextInt(rooms.length);
        messages = new ArrayList<>();
        wumpusMightBe = new ArrayList<>();
